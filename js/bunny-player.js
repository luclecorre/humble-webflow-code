function initBunnyPlayerBasic() {
  document.querySelectorAll('[data-bunny-player-init]').forEach(function(player) {
    ...
    var autoplay   = player.getAttribute('data-player-autoplay') !== 'false';
    ...
    // Media event wiring
    // setActivated on 'play' is safe (user intent is known); but status 'playing' must
    // wait for the 'playing' event so the first decoded frame is on-screen first.
    // This prevents the Safari black flash (play fires 300–500ms before first frame).
    video.addEventListener('play', function() { setActivated(true); });
